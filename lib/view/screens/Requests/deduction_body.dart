...
class DeductionBody extends StatefulWidget {
  int? selectIndex ;

  DeductionBody({super.key , this.selectIndex });


  @override
  State<DeductionBody> createState() =>DeductionBodyState();
}

class DeductionBodyState extends State<DeductionBody>  {
  ...
  _loadData() async {
    ...
    var data = await  Get.find<PermissionController>().getRequests(AppConstants.showDeductionRequest);
    if (data != 'error') {
      setState(() {
          ...
          Expanded(
            child:loader? const Center(child: CircularProgressIndicator()): ListView.separated(
                ...
                itemBuilder: (context, index) {
                  return RequestItem(index ,filteredData , type: "deduction_request".tr,) ;
                },
